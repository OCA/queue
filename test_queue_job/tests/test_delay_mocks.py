...
from odoo.addons.queue_job.tests.common import mock_jobs, mock_with_delay
...
class TestDelayMocks(common.SavepointCase):
    ...
    def test_mock_jobs_perform(self):
        with mock_jobs() as jobs_tester:
            ...
            node = Delayable(model).create_ir_logging("test_mock_jobs_perform graph 1")
            node2 = Delayable(model).create_ir_logging("test_mock_jobs_perform graph 2")
            node3 = Delayable(model).create_ir_logging("test_mock_jobs_perform graph 3")
            node2.done(node3)
            node3.done(node)
            node2.delay()
